...
#
# Copyright 2025, Marc S. Brooks (https://mbrooks.info)
...
[Unit]
Description=SCUM network issue workaround.
...
# Keep-alive process.
* * * * * if [[ ! \$(pgrep -f SCUM) ]]; then /bin/docker restart \$CONTAINER_ID > /dev/null; fi
EOF
